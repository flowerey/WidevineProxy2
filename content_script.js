      ...
      (target, thisArg, args) => {
        ...
        args[1] = async function (event) {
          if (
            ...
            const newChallenge = await emitAndWaitForResponse(
              "REQUEST",
              oldChallenge,
            );
  ...
  proxy(XMLHttpRequest.prototype, "send", (target, thisArg, args) => {
    thisArg.addEventListener("readystatechange", async () => {
      ...
      switch (thisArg.responseType) {
        ...
        case "json":
          body =
            typeof thisArg.response === "string"
              ? thisArg.response
              : JSON.stringify(thisArg.response);
          break;

        case "arraybuffer":
          if (
            thisArg.response &&
            thisArg.response.byteLength > 0 &&
            thisArg.response.byteLength < 1_000_000
          ) {
            ...
            body =
              arr.length <= 2000
                ? decoder.decode(arr)
                : decoder.decode(arr.slice(0, 1000)) +
                  decoder.decode(arr.slice(-1000));
          }
          break;

        case "blob":
          if (
            thisArg.response.type.startsWith("text/") ||
            thisArg.response.type.includes("xml") ||
            thisArg.response.type.includes("json") ||
            thisArg.response.size < 100_000
          ) {
