...
const { ClientIdentification, SignedDrmCertificate, DrmCertificate } =
...
export class WidevineDevice {
  ...
  get_name() {
    const client_info = Object.fromEntries(
      this.client_id.clientInfo.map((item) => [item.name, item.value]),
    );
