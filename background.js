...
const { LicenseType, SignedMessage, LicenseRequest, License } =
...
async function parseClearKey(body, sendResponse, tab_url) {
  ...
  const formatted_keys = clearkey["keys"].map((key) => ({
    ...key,
    kid: Util.bytesToHex(
      Util.b64.decode(key.kid.replace(/-/g, "+").replace(/_/g, "/") + "=="),
    ),
    k: Util.bytesToHex(
      Util.b64.decode(key.k.replace(/-/g, "+").replace(/_/g, "/") + "=="),
    ),
  }));
  const pssh_data = btoa(
    JSON.stringify({ kids: clearkey["keys"].map((key) => key.k) }),
  );
...
async function generateChallenge(body, sendResponse) {
  ...
  if (
    logs.filter(
      (log) => log.pssh_data === Session.psshDataToPsshBoxB64(pssh_data),
    ).length > 0
  ) {
    console.log(
      "[WidevineProxy2]",
      `KEYS_ALREADY_RETRIEVED: ${Util.b64.encode(pssh_data)}`,
    );
    sendResponse(body);
    return;
  }
  ...
  const device_b64 =
    await DeviceManager.loadWidevineDevice(selected_device_name);
  const widevine_device = new WidevineDevice(
    Util.b64.decode(device_b64).buffer,
  );
  ...
  const [challenge, request_id] = session.createLicenseRequest(
    LicenseType.STREAMING,
    widevine_device.type === 2,
  );
...
async function generateChallengeRemote(body, sendResponse) {
  ...
  const challenge_b64 = await remote_cdm.get_license_challenge(
    session_id,
    pssh,
    true,
  );

  const signed_challenge_message = SignedMessage.decode(
    Util.b64.decode(challenge_b64),
  );
  const challenge_message = LicenseRequest.decode(signed_challenge_message.msg);

  sessions.set(
    Util.b64.encode(challenge_message.contentId.widevinePsshData.requestId),
    {
      id: session_id,
      pssh: pssh,
    },
...
async function parseLicenseRemote(body, sendResponse, tab_url) {
  ...
  const keys = returned_keys.map(({ key, key_id }) => ({
    k: key,
    kid: key_id,
  }));
